...
Uses raw TOLD rewards and the learned online AMBI Q tail. Reward-only banks
therefore use the native TD-MPC2 return objective. Entropy-augmented banks keep
their learned soft-Q tail without an extra rollout or terminal entropy term.
Checkpoint actor temperature and Q normalization scale never change and do not
rescale the raw critic values used by planning.
"""
...
class FrozenAMBIMPPIController:
    ...
    def __init__(self, agent, settings=None):
        ...
        self.critic_target = str(agent.cfg.outer_critic_target)
        if self.critic_target not in {"reward_only", "entropy_augmented"}:
            raise ValueError("MPPI requires an explicit supported outer critic target.")
    ...
    @property
    def protocol(self):
        soft_q = self.critic_target == "entropy_augmented"
        return {
            ...
            "terminal_value_units": "raw_reward_soft_q" if soft_q else "raw_reward_q",
            "terminal_value_semantics": (
                "learned_soft_q_tail_without_entropy_correction" if soft_q
                else "learned_reward_only_q_tail"
            ),
            "source_critic_target": self.critic_target,
            "checkpoint_temperature": "frozen_unused_by_mppi",
            "checkpoint_q_scale": "frozen_no_planning_normalization",
