...
The native checkpoint supplies the frozen world model and control priors.
Action-local actor and critic copies can start from those priors or from random
weights; critic-only LoRA is optional. This wrapper has no outer-training or
...
# Keep the native objectives and paired-update schedule while allowing separate
# initialization choices and critic-only LoRA through the shared inner engine.
_NATIVE_CONTRACT = {
    "mpc": False,
...
def native_evaluation_params(params):
    ...
    result.setdefault("inner_critic_adaptation", "clone")
    ...
    result.setdefault("tdambi_scale_initialization", "checkpoint_or_calibrate")
...
class TDAMBIAgent(AMBITDMPC2Agent):
    ...
    def __init__(self, cfg):
        super().__init__(cfg)
        self.register_buffer("tdambi_checkpoint_scale", None)

    @property
    def tdambi_scale_source(self):
        if (
            self.cfg.tdambi_scale_initialization != "calibrate"
            and self.tdambi_checkpoint_scale is not None
        ):
            return "checkpoint"
        return "first_collection_calibration"
    ...
    def load(self, fp):
        ...
        saved_scale = preflight_scale_state(state["scale"]) if "scale" in state else None
        if saved_scale is None and self.cfg.tdambi_scale_initialization == "checkpoint":
            raise ValueError(
                "TDAMBI tdambi_scale_initialization='checkpoint' requires a saved "
                "Q scale. This legacy checkpoint has none; use 'calibrate' or "
                "'checkpoint_or_calibrate' for the historical fallback."
            )
        ...
        self.model.eval()
        # Keep a frozen reference for every future root. Never alias checkpoint
        # tensors supplied by a caller or the action-local mutable estimator.
        self.tdambi_checkpoint_scale = (
            None if saved_scale is None
            else saved_scale["value"].detach().to(self.device).clone()
        )
...
class TDAMBI(AMBITDMPC2):
    """Frozen native TD-MPC2 backbone with a fresh actor--critic per action."""
    ...
    def _build_cfg(self, params):
        if str(params.get("inner_sac_return_estimator", "one_step")).lower() != "one_step":
            raise ValueError("Native TDAMBI requires inner_sac_return_estimator='one_step'.")
        ...
        params["inner_critic_adaptation"] = str(params["inner_critic_adaptation"]).lower()
        if params["inner_critic_adaptation"] not in {"clone", "lora_rl"}:
            raise ValueError("TDAMBI inner_critic_adaptation must be 'clone' or 'lora_rl'.")
        online_target = (
            params["inner_critic_initialization"] == "random"
            or params["inner_critic_adaptation"] == "lora_rl"
        )
        target_initialization = str(params.get(
            "inner_critic_target_initialization",
            "online" if online_target else "outer_target",
        )).lower()
        ...
        if online_target and target_initialization != "online":
            raise ValueError(
                "TDAMBI random or LoRA critics require "
                "inner_critic_target_initialization='online'."
            )
        # Historically the shared config reported 'online', but native TDAMBI
        # always copied the saved outer target. Normalize that legacy spelling
        # for prior+clone without changing its behavior or publication identity.
        params["inner_critic_target_initialization"] = (
            "online" if online_target else "outer_target"
        )
        scale_initialization = str(params["tdambi_scale_initialization"]).lower()
        if scale_initialization not in {"checkpoint_or_calibrate", "checkpoint", "calibrate"}:
            raise ValueError(
                "TDAMBI tdambi_scale_initialization must be 'checkpoint_or_calibrate', "
                "'checkpoint', or 'calibrate'."
            )
        params["tdambi_scale_initialization"] = scale_initialization
