...
def test_retrace_rejects_ere_instead_of_ignoring_the_schedule():
    with pytest.raises(ValueError, match="one-step inner SAC"):
        _build_cfg(inner_replay_strategy="ere", inner_sac_return_estimator="retrace",
                   inner_finite_horizon=True, inner_rollouts_per_round=128,
                   inner_critic_updates_per_round=16, inner_actor_updates_per_round=4)


@pytest.mark.parametrize("horizon", [1, 2, 3])
def test_j1_return_critic_campaign_is_exactly_uniform_including_rng(models, horizon):
    """One retained collection round permits reuse of historical uniform J1."""
    options = dict(
        aux_return_mode="sac", aux_return_detach_representation=False,
        target_entropy=-10.5, inner_critic_source="aux_return",
        inner_horizon_critic_source="aux_return", inner_sac_critic_target="reward_only",
        inner_finite_horizon=True, inner_terminal_entropy="none",
        inner_rounds=1, inner_rollouts_per_round=128, inner_rollout_horizon=horizon,
        train_unroll_horizon=3, inner_replay_capacity=3840, inner_batch_size=256,
        inner_critic_updates_per_round=16, inner_actor_updates_per_round=4,
        inner_actor_lr=.0003, inner_critic_lr=.0003, inner_temperature_lr=.0003,
        inner_critic_target_tau=.01, q_representation="distributional", num_q=5,
        q_num_bins=101, dropout=.01,
    )
    uniform = models(inner_replay_strategy="uniform", **options)
    ere = models(inner_ere_final_fraction=.25, inner_ere_min_rounds=1,
                 inner_ere_actor=True, **options)
    global_rng = torch.random.get_rng_state().clone()
    for observation in (torch.zeros(3), torch.tensor([.1, -.2, .3])):
        baseline_trace, ere_trace = InnerActionTrace(), InnerActionTrace()
        expected = uniform.agent.act(observation, collect_diagnostics=False,
                                     trace=baseline_trace)
        actual = ere.agent.act(observation, collect_diagnostics=False, trace=ere_trace)
        ...
        assert len(ere_trace.events) == len(baseline_trace.events)
        for left, right in zip(ere_trace.events, baseline_trace.events):
            # Replay telemetry is the only additional scientific trace content.
            left = deepcopy(left)
            for name in list(left['metrics']):
                if name.startswith(('critic_replay_', 'actor_replay_',
                                    'inner_critic_replay_', 'inner_actor_replay_')):
                    left['metrics'].pop(name)
            _assert_tree_equal(left, right)
        metrics = ere.agent.last_inner_metrics
        assert metrics['inner_critic_optimizer_steps'] == 16
        assert metrics['inner_actor_optimizer_steps'] == 4
        assert metrics['inner_temperature_optimizer_steps'] == 4
        assert metrics['inner_critic_replay_round_1_sample_count'] == 16 * 256
        assert metrics['inner_actor_replay_round_1_sample_count'] == 4 * 256
...
def test_component_windows_depth_eligibility_and_budgets(models,monkeypatch,order,sampling):
    model=models(inner_component_update_order=order,
