...
from slurm.ambi_aux_closed_loop import MATRIX, MATRICES, SELECTOR, checkpoint_alpha, matrix_for, validate_bundle, UPDATE_BUDGETS
...
def test_h2_update_matrices_change_only_update_counts(budget, critic, actor):
    base = json.loads(matrix_for('auto', 2).read_text())
    updated = json.loads(matrix_for('auto', 2, budget).read_text())
    assert updated['shared_alg_params'] == {
        **base['shared_alg_params'], 'inner_critic_updates_per_round': critic,
        'inner_actor_updates_per_round': actor,
    }
    for key in set(base) - {'description', 'shared_alg_params'}:
        assert updated[key] == base[key]
    for alpha, horizon in [('zero', 2), ('inherit_outer', 2), ('auto', 1)]:
        with pytest.raises(ValueError, match='require H2'):
            matrix_for(alpha, horizon, budget)
...
def test_update_budget_validation_rejects_wrong_counts_and_cross_budget_labels(panel, tmp_path):
    ...
    if horizon != 2:
        return
    original = json.loads((root / 'serial/manifest.json').read_text())
    other = 'c64_a4' if update_budget == 'c32_a4' else 'c32_a4'
    with pytest.raises(AssertionError):
    ...
    for metric in ('inner_critic_optimizer_steps', 'inner_actor_optimizer_steps',
        ...
        manifest = copy.deepcopy(original)
        manifest['runs'][0]['result']['model_metrics'][metric].update(mean=0, min=0, max=0)
        (tmp_path / 'manifest.json').write_text(json.dumps(manifest))
        with pytest.raises(AssertionError):
            validate_bundle(tmp_path, [101, 102], 3, expected_alpha=alpha,
                            alpha_mode=alpha_mode, critic_mode=critic_mode, horizon=2,
