...
assert.equal(api.capabilities(data.runs).optimizer, true);
const decisionRun = structuredClone(data.runs[0]);
decisionRun.roots = [];
decisionRun.traces = decisionRun.traces.filter(t => t.mode === "episodes").map(t => ({
 ...t, event_index:[0], round_index:[2], critic_updates:[8], actor_updates:[3],
 temperature_updates:[3], phase:["decision"], metrics:{"decision/reward":[50+t.decision_index]}, nonfinite:{}
}));
decisionRun.diagnostic_capabilities = {decision_metrics:true, optimizer_traces:false, shared_observation_probes:false};
assert.deepEqual(api.capabilities([decisionRun]), {axes:["decision_index"], modes:["episodes"], optimizer:false});
const decisionPoints = api.overlay([decisionRun], "episodes", episode, 0, "decision/reward", "decision_index")[0].points;
assert.deepEqual(decisionPoints.map(p => [p.x,p.value,p.critic,p.actor,p.temperature]), [[0,50,8,3,3],[1,51,8,3,3]]);
