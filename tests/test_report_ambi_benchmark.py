...
def test_togo_metadata_is_retained_and_incompatible_probes_do_not_overlay(tmp_path):
    first = _bundle(tmp_path / "first")
    first["runs"][0].update(togo_return_probe={"version": 2, "rollouts": 32},
                            togo_probe_rows=[{"round_index": 0}],
                            togo_round_summaries=[{"round_index": 0, "mean": 12.}])
    _save_manifest(tmp_path / "first", first)
    loaded = report.load_bundles([tmp_path / "first"])["runs"][0]
    assert loaded["togo_return_probe"] == {"version": 2, "rollouts": 32}
    assert loaded["togo_probe_rows"] == [{"round_index": 0}]
    assert loaded["togo_round_summaries"] == [{"round_index": 0, "mean": 12.}]
    second = _bundle(tmp_path / "second", evaluation="eval-2")
    second["runs"][0]["togo_return_probe"] = {"version": 2, "rollouts": 64}
    _save_manifest(tmp_path / "second", second)
    with pytest.raises(ValueError, match="Incompatible to-go probe"):
