...
def tdambi_label_registry(**settings):
    registry = label_registry()
    registry["identity"]["backbone"] = "rwgao_b-brown-university/ambi/xq3zva9u"
    registry["identity"]["planner"] = {"type": "tdambi", "settings": {
        "inner_updates_per_round": 3, "inner_rounds": 6, "inner_rollouts_per_round": 512,
        "inner_rollout_horizon": 3, "inner_batch_size": 512, "inner_actor_lr": 3e-4,
        "inner_critic_lr": 3e-4, "tdambi_entropy_coef": 1e-4, "tdambi_scale_tau": .01,
        **settings}}
    return registry


def test_tdambi_names_and_legends_identify_native_budget_without_changing_identity():
    registry = tdambi_label_registry()
    before = deepcopy(registry)
    assert series.concise_curve_label(registry) == "TD-MPC2 · TDAMBI C3/A3 · #abcd"
    name = series.evaluation_run_name(registry)
    assert "TD-MPC2 prior-only backbone | TDAMBI C3/A3 J6 N512 H3 B512" in name
    assert "fixed native entropy eta=0.0001" in name
    assert "reward-only inner Q; Q-only scale: first replay + EMA tau=0.01" in name
    assert registry == before


def test_tdambi_compact_legend_keeps_nondefault_native_settings():
    registry = tdambi_label_registry(inner_updates_per_round=12, inner_rounds=8,
                                    inner_rollouts_per_round=1024, inner_rollout_horizon=5,
                                    inner_batch_size=256, inner_actor_lr=1e-4,
                                    tdambi_entropy_coef=.001, tdambi_scale_tau=.1)
    label = series.concise_curve_label(registry)
    assert "TDAMBI C12/A12 J8 N1024 H5 B256 aLR0.0001 eta0.001 scaleTau0.1" in label


def test_tdambi_actual_publisher_uses_native_name_legend_and_tags(tmp_path):
    value = record(tmp_path)
    value["identity"].update(tdambi_label_registry()["identity"])
    registry = create(tmp_path, value)
    before = deepcopy(registry["identity"])
    series.stage_record(registry["run_dir"], value)
    backend = FakeWandb()
    publish(registry, backend)
    call = backend.init_calls[0]
    assert "TDAMBI C3/A3 J6 N512 H3 B512" in call["name"]
    assert "TDAMBI C3/A3" in call["config"]["curve_label"]
    assert call["tags"] == ["evaluation-curve", "eval-series-v1", "tdambi", "native-tdmpc2",
                            "fixed-native-entropy", "q-only-scale", "prior-only-backbone", "C3/A3"]
    assert registry["identity"] == before
    assert series._evaluation_run_tags(label_registry()) == ["evaluation-curve", "eval-series-v1"]
    unknown = tdambi_label_registry()
    unknown["identity"]["backbone"] = "another-entity/ambi/xq3zva9u"
    assert "prior-only-backbone" not in series._evaluation_run_tags(unknown)
