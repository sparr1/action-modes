...
@pytest.mark.parametrize("adaptation,scale,critic_rank,expected", [
    ("clone", 1.0, 8, "aea9075b9d13cee5850f2f816445973f9d23b37e96cc85e2179b4167ffa6f42b"),
    ("lora", 2.0, 16, "5866eee0effb59c240df8a8014e3eee4bc2cb41fcdce4bfddfa9150b84034a4f"),
])
def test_historical_dense_and_lora_planner_hashes_unchanged(adaptation, scale, critic_rank, expected):
    # Pre-lora_rl fingerprints pin both the retired method and dense planners.
    config = {"inner_operator": "sac", "inner_actor_adaptation": adaptation,
              "inner_critic_adaptation": adaptation, "inner_actor_lora_rank": 8,
              "inner_actor_lora_scale": scale, "inner_actor_lora_dropout": 0.0,
              "inner_critic_lora_rank": critic_rank, "inner_critic_lora_scale": scale,
              "inner_critic_lora_dropout": 0.0}
    planner = data.planner_identity(config, {}, "AMBITDMPC2/AMBITDMPC2", "tanh_mean")
    encoded = json.dumps(planner, sort_keys=True, separators=(",", ":"))
    assert hashlib.sha256(encoded.encode()).hexdigest() == expected


def test_lora_rl_defaults_normalize_and_inactive_knobs_do_not_split_dense_publication(bundle):
    path, payload = bundle
    ...
    config.update(inner_critic_lora_layers="input_hidden", inner_critic_lora_rank=96,
                  ...
                  inner_actor_lora_rank=8, inner_critic_lora_dropout=0.3)
    explicit_dense, = data.load_records(dump(path, payload))
    assert dense["identity"] == explicit_dense["identity"]

    config["inner_critic_adaptation"] = "lora_rl"
    explicit_lora, = data.load_records(dump(path, payload))
    for key in list(config):
        if "_lora_" in key:
            config.pop(key)
    config.pop("inner_actor_adaptation")
    implicit_lora, = data.load_records(dump(path, payload))
    assert implicit_lora["identity"] == explicit_lora["identity"]
    assert implicit_lora["identity"] != dense["identity"]
    settings = implicit_lora["identity"]["planner"]["settings"]
    assert settings["inner_actor_adaptation"] == "clone"
    assert settings["inner_critic_adaptation"] == "lora_rl"
    assert settings["inner_critic_lora_layers"] == "input_hidden"
    assert settings["inner_critic_lora_rank"] == 96
    assert settings["inner_critic_lora_scale"] == 1.0
    assert settings["inner_critic_lora_weight_decay"] == 0.0002
...
def test_lora_rl_active_choices_split_publication_identity(bundle, field, value):
    path, payload = bundle
    config = payload["runs"][0]["resolved_config"]
    config["inner_critic_adaptation"] = "lora_rl"
    baseline, = data.load_records(dump(path, payload))
    config[field] = value
    changed, = data.load_records(dump(path, payload))
    assert baseline["identity"]["planner"] != changed["identity"]["planner"]
