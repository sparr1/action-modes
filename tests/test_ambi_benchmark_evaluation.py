...
def test_frozen_evaluator_keeps_step_timing_and_full_trace(checkpoint_matrix, tmp_path):
    import evaluate_ambi_checkpoint as evaluator
    from report_ambi_benchmark import load_bundles
    ...
    matrix_path.write_text(json.dumps(matrix))
    bundle = tmp_path / "step-evaluation"
    result = evaluator.evaluate_matrix(
        matrix_path, checkpoint, selectors=["budget/sac"], bundle_dir=bundle,
        max_steps=1, seeds=[101],
    )["results"][0]
    assert result["outer_state_unchanged"]
    assert result["resolved_config"]["inner_update_timing"] == "step"
    rows = events(bundle)
    assert [row["phase"] for row in rows] == ["initial"] + ["collection", "update"] * 4 + ["decision"]
    load_bundles([bundle])
