...
def _cuda_trace_benchmark_setup():
    ...
    if checkpoint_value:
        ...
        matrix = Path(os.environ.get(
            "AMBI_TRACE_BENCHMARK_MATRIX",
            Path(__file__).resolve().parents[1] / "configs/research/ambi_humanoid_inner_benchmark.json",
        )).expanduser().resolve()
        selector = os.environ.get("AMBI_TRACE_BENCHMARK_PRESET", "inner_budget/sac_1x")
        controller_seed = int(os.environ.get("AMBI_TRACE_BENCHMARK_CONTROLLER_SEED", "55"))
        context = load_checkpoint_context(checkpoint)
        resolved = resolve_preset(
            matrix, selector, checkpoint_context=context,
        )
        env = _make_env(resolved)
        model = None
        try:
            model, _ = _initialize_frozen_model(
                resolved, env, checkpoint, controller_seed=controller_seed, device="cuda",
            )
            ...
            return model, model._obs_to_tensor(observation), {
                ...
                "selector": selector, "controller_seed": controller_seed,
...
@pytest.mark.parametrize("tdambi", [False, True])
def test_actual_checkpoint_benchmark_uses_sidecar_and_research_preset(monkeypatch, tmp_path, tdambi):
    ...
    if tdambi:
        matrix = tmp_path / "tdambi-matrix.json"
        matrix.write_text("{}")
        monkeypatch.setenv("AMBI_TRACE_BENCHMARK_MATRIX", str(matrix))
        monkeypatch.setenv("AMBI_TRACE_BENCHMARK_PRESET", "inner_budget/tdambi_3")
        monkeypatch.setenv("AMBI_TRACE_BENCHMARK_CONTROLLER_SEED", "12345")
    else:
        for key in ("MATRIX", "PRESET", "CONTROLLER_SEED"):
            monkeypatch.delenv("AMBI_TRACE_BENCHMARK_" + key, raising=False)
    ...
    def initialize(resolved, actual_env, actual_checkpoint, **kwargs):
        ...
        assert kwargs == {"controller_seed": 12345 if tdambi else 55, "device": "cuda"}
        return model, {}
    ...
    assert calls["selector"] == ("inner_budget/tdambi_3" if tdambi else "inner_budget/sac_1x")
    assert calls["matrix"].name == ("tdambi-matrix.json" if tdambi else "ambi_humanoid_inner_benchmark.json")
...
def test_cuda_trace_overhead_measurement(capsys):
    ...
    cuda_elapsed, calibration_elapsed = ({False: [], True: []} for _ in range(2))
    traces = []
    engine = model.agent.inner_engine
    try:
        ...
        for repeat in range(10):
            for enabled in ((False, True) if repeat % 2 == 0 else (True, False)):
                ...
                cuda_start, cuda_end = (torch.cuda.Event(enable_timing=True) for _ in range(2))
                stream = torch.cuda.current_stream(model.agent.device)
                ...
                cuda_elapsed[enabled].append(cuda_start.elapsed_time(cuda_end) / 1000)
                calibration_elapsed[enabled].append(float(
                    model.agent.last_inner_metrics.get("inner_tdambi_calibration_seconds", 0.0)
                ))
                ...
                # Timing synchronization belongs to this benchmark, not the
                # controller or CPU append measurement.
                append_elapsed[enabled].append(0.0 if trace is None else finished - append_started)
                elapsed[enabled].append(control_done - started + append_elapsed[enabled][-1])
        ...
        ordinary_cuda, traced_cuda = (statistics.median(cuda_elapsed[value]) for value in (False, True))
        serialization_per_action = serialization / len(traces)
        result = {
            **provenance, "device": torch.cuda.get_device_name(model.agent.device),
            ...
            "compilation_enabled": bool(model.cfg.compile),
            "compilation_seconds": None if model.cfg.compile else 0.0,
            ...
            "ordinary_cuda_seconds": ordinary_cuda, "traced_cuda_seconds": traced_cuda,
            "cuda_trace_overhead_fraction": traced_cuda / ordinary_cuda - 1,
            "cuda_target_met": traced_cuda / ordinary_cuda - 1 <= 0.05,
            "ordinary_calibration_seconds": statistics.median(calibration_elapsed[False]),
            "traced_calibration_seconds": statistics.median(calibration_elapsed[True]),
            "diagnostic_probe_seconds": 0.0, "upload_seconds": None,
            "upload_status": "not_performed",
            "serialization_seconds": serialization, "serialized_bytes": len(payload.encode()),
            "compressed_bytes": len(compressed),
            "serialization_seconds_per_action": serialization_per_action,
            "traced_with_serialization_seconds": traced + serialization_per_action,
            "total_local_overhead_fraction": (traced + serialization_per_action) / ordinary - 1,
            "samples": {
                ...
                "ordinary_cuda_seconds": cuda_elapsed[False],
                "traced_cuda_seconds": cuda_elapsed[True],
                "ordinary_calibration_seconds": calibration_elapsed[False],
                "traced_calibration_seconds": calibration_elapsed[True],
            },
        }
        if output_path is not None:
