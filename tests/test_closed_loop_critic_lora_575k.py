"""Pin critic-only LoRA scope, exact dense controls, and new publication identities."""
...
from slurm import ambi_closed_loop_critic_lora as campaign
...
def test_complete_scope_and_no_reused_lora_measurements():
    panel = campaign.cells()
    expected = [(rank, h, j) for rank in (16, 96) for h in (1, 2, 3) for j in (1, 2, 4, 6, 8, 10)]
    assert [(c['lora_rank'], c['H'], c['J']) for c in panel] == expected
    assert len(panel) == len({c['name'] for c in panel}) == 36
    assert not any(c['reused'] for c in panel)
    assert len(panel)*len(campaign.SEEDS) == 180
    for cell in panel:
        p = cell['params']
        assert {k: p[k] for k in campaign.ERE_DEFAULTS} == campaign.ERE_DEFAULTS
        assert {k: p[k] for k in campaign.lora_settings(cell['lora_rank'])} == campaign.lora_settings(cell['lora_rank'])
        assert p['inner_actor_adaptation'] == 'clone'
        ...
        assert p['inner_actor_writeback_coef'] == p['inner_critic_writeback_coef'] == 0
...
def test_resolved_update_budget_and_replay_scope(cell):
    ...
    assert cfg.inner_replay_strategy == 'uniform'
    assert cfg.inner_actor_adaptation == 'clone' and cfg.inner_critic_adaptation == 'lora_rl'
    assert cfg.inner_critic_lora_rank == cell['lora_rank']
    assert cfg.inner_adaptation == 'mixed'
    for component in ('actor', 'critic', 'temperature', 'replay', 'actor_optimizer', 'critic_optimizer', 'temperature_optimizer'):
        assert getattr(cfg, 'inner_' + component + '_scope') == 'action'


@pytest.mark.parametrize('mutation', ['missing', 'duplicate', 'rank', 'scale', 'decay', 'placement', 'actor', 'replay', 'capacity', 'critic', 'execution', 'retrace'])
def test_rejects_scope_or_scientific_drift(tmp_path, mutation):
    ...
    if mutation == 'missing': selectors.pop()
        ...
        key, value = {'rank': ('inner_critic_lora_rank', 16), 'scale': ('inner_critic_lora_scale', .5),
            'decay': ('inner_critic_lora_weight_decay', 0.), 'placement': ('inner_critic_lora_layers', 'hidden'),
            'actor': ('inner_actor_adaptation', 'frozen'), 'replay': ('inner_replay_strategy', 'ere'),
            'capacity': ('inner_replay_capacity', 3072), 'critic': ('inner_critic_source', 'sac'),
            'execution': ('inner_eval_execution_action', 'policy_sample'),
...
@pytest.mark.parametrize('rank', [16, 96])
def test_old_defaults_normalize_without_allowing_unrelated_differences(rank):
    ...
    actual = {**old, 'inner_eval_execution_action': 'mean', 'inner_sac_return_estimator': 'one_step',
              ...
              **campaign.ERE_DEFAULTS, **campaign.lora_settings(rank)}
    campaign.matching_uniform_config(actual, old, rank)
    campaign.matching_uniform_config({**actual, 'device': 'cpu'}, {**old, 'device': 'cuda'}, rank)
    campaign.matching_uniform_config({**actual, 'inner_adaptation': 'mixed'}, {**old, 'inner_adaptation': 'clone'}, rank)
    for key, value in [('inner_actor_lr', .01), ('inner_replay_capacity', 3072),
                       ('inner_critic_lora_rank', 96 if rank == 16 else 16), ('inner_critic_lora_scale', .5),
                       ('inner_critic_lora_layers', 'hidden'), ('inner_critic_lora_weight_decay', 0.),
                       ('inner_actor_adaptation', 'frozen'), ('inner_replay_strategy', 'ere'),
                       ('inner_temperature_initialization', 'fixed')]:
        with pytest.raises(AssertionError): campaign.matching_uniform_config({**actual, key: value}, old, rank)
    with pytest.raises(AssertionError):
        campaign.matching_uniform_config(actual, {**old, 'inner_replay_strategy': 'ere'}, rank)
    with pytest.raises(AssertionError):
        campaign.matching_uniform_config({**actual, 'inner_adaptation': 'clone'}, old, rank)


@pytest.mark.parametrize('failure', [None, 'model_fallback', 'decision_fallback', 'outer_state', 'rank', 'actor', 'trace_coverage'])
def test_completed_bundle_rejects_fallback_or_scientific_drift(tmp_path, monkeypatch, failure):
    import gzip
    import json
    cell = campaign.cells()[0]
    cell['uniform_reference'] = {'resolved_config': campaign.historical_cell(1, 1)['params']}
    config = deepcopy(cell['params'])
    result = dict(outer_state_unchanged=True,
                  model_metrics={'inner_compile_fallback': dict(mean=0, min=0, max=0)})
    rows = [{'phase': 'update', 'metrics': {}} for _ in range(3*20)]
    rows.extend({'phase': 'decision', 'metrics': {'decision/inner_compile_fallback': 0}} for _ in range(3))
    if failure == 'model_fallback': result['model_metrics']['inner_compile_fallback']['max'] = 1
    elif failure == 'decision_fallback': rows[-1]['metrics']['decision/inner_compile_fallback'] = 1
    elif failure == 'outer_state': result['outer_state_unchanged'] = False
    elif failure == 'rank': config['inner_critic_lora_rank'] = 96
    elif failure == 'actor': config['inner_actor_adaptation'] = 'frozen'
    elif failure == 'trace_coverage': rows.pop(0)
    with gzip.open(tmp_path / 'trace.jsonl.gz', 'wt') as handle:
        for row in rows: handle.write(json.dumps(row) + '\n')
    manifest = {'runs': [dict(resolved_config=config, result=result, trace_files=['trace.jsonl.gz'])]}
    monkeypatch.setattr(campaign, 'validate_checkpoint', lambda *a, **k: manifest)
    monkeypatch.setattr(campaign, 'verify_reference', lambda *a, **k: {})
    if failure is None:
        assert campaign.validate_completed(tmp_path, cell, {}, smoke=True) is manifest
    else:
        with pytest.raises(AssertionError): campaign.validate_completed(tmp_path, cell, {}, smoke=True)
...
def test_prepare_creates_thirty_six_new_identities_and_never_republishes_controls(tmp_path, monkeypatch, late_config_drift):
    ...
    assert len(created) == len(set(created)) == 36
    assert state['production_indices'] == list(range(36)) and state['smoke_indices'] == [0, 17, 18, 35]
    assert len(state['uniform_references']) == 18 and state['replay_strategy'] == 'uniform'
    assert state['ranks'] == [16, 96]
    for cell in state['cells']:
        ...
        assert cell['identity'] != old['identity']


def test_prepare_uses_real_checkpoint_metadata_specs_and_registry(tmp_path, monkeypatch):
    """Exercise actual config/identity resolution for all 36 cells without live evaluation."""
    import subprocess
    import torch
    ...
    assert len(list(args.registry.iterdir())) == 36
    assert not (args.root / 'unused').exists()
    for cell in result['cells']:
        spec = campaign.read(args.root / 'specs' / (cell['selector'].replace('/', '__') + '.json'))
        assert spec['identity']['planner']['settings'].get('inner_replay_strategy', 'uniform') == 'uniform'
        assert spec['identity']['planner']['settings']['inner_critic_lora_rank'] == cell['lora_rank']
        assert spec['identity']['planner']['settings']['inner_critic_adaptation'] == 'lora_rl'
