...
def test_mismatched_curve_assignment_fails_before_environment(checkpoint_matrix, tmp_path, monkeypatch):
...
def test_matching_curve_assignment_queues_after_frozen_evaluation(checkpoint_matrix, tmp_path, monkeypatch):
    checkpoint, matrix = checkpoint_matrix
    registry = core.create_run(tmp_path / "registry", {"identity": identity(), "label": "prior"},
                               "first", "project", "entity", "owner")
    ...
    assert pointer["selector"] == "budget/prior"
    assert json.loads((Path(registry["run_dir"]) / "publication.json").read_text())["records"] == {}


def test_prelaunch_specification_does_not_create_environment(checkpoint_matrix, tmp_path, monkeypatch):
    checkpoint, matrix = checkpoint_matrix
    ...
    spec = json.loads(Path(result["specs"]["budget/prior"]).read_text())
    assert spec["identity"] == identity()


def test_banked_diagnostics_reject_curve_assignment_before_compute(checkpoint_matrix, tmp_path, monkeypatch):
    checkpoint, matrix = checkpoint_matrix
    registry = core.create_run(tmp_path / "registry", {"identity": identity(), "label": "prior"},
                               "first", "project", "entity", "owner")
