...
STEPS = tuple(range(100_000, 400_001, 50_000))
...
def load_result(path, source_run, campaign):
    ...
    step = int(match[1])
    _require(step in STEPS, "Checkpoint is outside the 100k–400k campaign")
    result = _read(path)
...
def campaign_data(root, source_run, campaign):
    ...
    signature = lambda item: (item["result"]["protocol"], item["result"]["planner"],
                              ...
                              item["provenance"]["code_sha"], item["provenance"]["code_tree"])
...
def publish(path, *, source_run, campaign, project="ambi-inner-bench",
            entity="rwgao_b-brown-university", wandb_module=None):
    ...
    with (root / ".wandb-publication.lock").open("a") as lock:
        ...
        loaded, curves = campaign_data(root, source_run, campaign)
        if disabled:
