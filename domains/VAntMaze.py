...
from domains.ant_variable_legs import set_num_legs

class VAntMazeEnv(MazeEnv, EzPickle):
    metadata = {
        "render_modes": [
            "human",
            "rgb_array",
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #TODO we need to debug the fact that AntVariableLegs thinks it will be init second (receiving a temp xml), while 
        #at the same time, MazeEnv (super) also thinks it will be init second (to receive the correct ant_xml file)
        # self.ant_env = AntVariableLegsEnv(
        ...
        # ant_xml_file_path = "domains/assets/ant_6_legs.xml"
        xml_template = path.join(path.dirname(__file__), "assets", "ant_variable_legs_template.xml")
        xml_file = path.join(path.dirname(__file__), "assets", "ant_variable_legs.xml")
        set_num_legs(xml_template, xml_file, num_legs=num_legs)

        super().__init__(
            agent_xml_path=xml_file,
            maze_map=maze_map,
            maze_size_scaling=4,
            maze_height=0.5,
