...
def _load_run(directory, manifest, run, run_key):
    ...
    for solve_key, solve in sorted(solves.items()):
        ...
        traces.append(solve)
    capabilities = run.get("diagnostic_capabilities", {
        "decision_metrics": any("decision" in trace["phase"] for trace in traces),
        "optimizer_traces": any(any(phase != "decision" for phase in trace["phase"]) for trace in traces),
        "shared_observation_probes": bool(roots),
    })
    if capabilities.get("optimizer_traces") is False and any(
        trace["phase"] != ["decision"] for trace in traces
    ):
...
def render_html(data, *, title="AMBI inner optimization benchmark"):
    ...
    # Omit unavailable choices from the delivered HTML as well as the live UI.
    capabilities = [run.get("diagnostic_capabilities", {}) for run in data["runs"]]
    if not any(item.get("shared_observation_probes", True) for item in capabilities):
        template = template.replace('<option value="bank">Shared roots</option>', "")
        template = re.sub(r'<p data-bank-detail>.*?</p>', "", template)
    if not any(item.get("optimizer_traces", True) for item in capabilities):
        template = re.sub(r'<option value="(?:event_index|critic_updates|actor_updates|temperature_updates|round_index)">[^<]*</option>', "", template)
        template = re.sub(r'<p data-optimizer-detail>.*?</p>', "", template)
        template = template.replace("Selected metric across inner updates", "Selected metric across real decisions")
