...
def _load_run(directory, manifest, run, run_key):
    ...
    return {"key": run_key, "id": run_id, "evaluation_id": manifest["evaluation_id"],
            ...
            **{key: run[key] for key in ("togo_return_probe", "togo_probe_rows", "togo_round_summaries")
               if key in run}}
...
def load_bundles(bundle_paths):
    ...
    for supplied in bundle_paths:
        ...
        for run in manifest.get("runs", []):
            ...
            if run.get("togo_return_probe") is not None:
                signature = _canonical(run["togo_return_probe"])
                if togo_signature is not None and signature != togo_signature:
                    raise ValueError("Incompatible to-go probe protocol across report runs.")
                togo_signature = signature
