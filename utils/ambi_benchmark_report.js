...
(function (root) {
  ...
  function capabilities(runs) {
    const optimizer = runs.some(run => run.diagnostic_capabilities ? run.diagnostic_capabilities.optimizer_traces
      : run.traces.some(trace => trace.phase.some(phase => phase !== "decision")));
    const bank = runs.some(run => run.diagnostic_capabilities ? run.diagnostic_capabilities.shared_observation_probes : run.roots.length > 0);
    return {axes: optimizer ? Object.keys(axes) : ["decision_index"], modes: bank ? ["episodes", "bank"] : ["episodes"], optimizer};
  }
  const api = {series, selectTraces, selections, extent, heatmapCells, overlay, episodeKey, capabilities};
  ...
  function boot(data) {
    ...
    function refreshOptions() {
      ...
      const available = capabilities(selected.length ? selected : data.runs);
      replaceOptions($("mode"), available.modes.map(key => ({key, label: key === "bank" ? "Shared roots" : "Evaluation episodes"})), $("mode").value);
      replaceOptions($("axis"), available.axes.map(key => ({key, label: axes[key]})), $("axis").value);
      $("mode").closest("label").hidden = available.modes.length === 1;
      $("axis").closest("label").hidden = available.axes.length === 1;
    ...
    function curves() {
      ...
      entries.forEach(entry => {
        ...
        entry.points.forEach(point => {
          ...
          const boundary = point.phase === "decision" ? "completed solve" : `event ${point.event} · ${point.phase} · round ${point.round}`;
          const description = `${entry.run.label} · ${metric}=${point.nonfinite || format(point.value)} · decision ${point.decision} · ${boundary} · actual critic ${point.critic} / actor ${point.actor} / temperature ${point.temperature}`;
    ...
    const preferred = () => {const value = (data.metric_catalog[$("metric").value] || {}).preferred_axis;
      const available = capabilities(chosen().length ? chosen() : data.runs).axes;
      $("axis").value = available.includes(value) ? value : available[0];};
