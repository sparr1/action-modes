...
def backbone_display_label(identity, *, compact=False):
    ...
    known = {
        "rwgao_b-brown-university/ambi/aux6434715x3": ("AMBI auxiliary return · target -21 detached · seed55", "Aux return -21 detached"),
...
def _planner_display_label(planner, *, compact=False):
    ...
    if kind in ("sac", "xqc"):
        ...
        for value, prefix in ((settings.get("planning_horizon", settings.get("horizon", settings.get("inner_rollout_horizon"))), "H"),
                              (settings.get("num_samples", settings.get("inner_mppi_num_samples")), "N"),
                              (settings.get("num_elites", settings.get("inner_mppi_num_elites")), "E"),
                              (settings.get("num_pi_trajs", settings.get("inner_mppi_num_pi_trajs")), "pi"),
                              (settings.get("effective_iterations", settings.get("inner_mppi_iterations")), "I")):
            if value is not None and (not compact or prefix in ("H", "N", "I")):
                parts.append(prefix + number(value))
        if "inner_horizon_critic_source" in settings or "inner_mppi_iterations" in settings:
            parts.append("return-only Q" if settings.get("inner_horizon_critic_source", "sac") == "aux_return" else "soft Q")
        if not compact:
