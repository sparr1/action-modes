...
def _planner_display_label(planner, *, compact=False):
    ...
    if kind in ("sac", "xqc"):
        ...
        for key, prefix, default in (("inner_rounds", "J", 6),
                                     ...
                                     ("inner_batch_size", "B", 512)):
            value = settings.get(key)
            if value is not None and (not compact or value != default):
                parts.append(prefix + number(value))
        for key, prefix in (("inner_critic_lr", "cLR"), ("inner_actor_lr", "aLR")):
            value = settings.get(key)
        ...
        if not compact:
            parts.append("reward-only inner Q; Q-only scale: first replay + EMA" +
                         (" tau=" + number(scale_tau) if scale_tau is not None else ""))
        elif scale_tau is not None and scale_tau != .01:
            parts.append("scaleTau" + number(scale_tau))
    elif kind == "mppi":
...
def _evaluation_run_tags(registry):
    tags = ["evaluation-curve", "eval-series-v1"]
    identity = registry["identity"]
...
class Publisher:
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.run = self.wandb.init(id=self.registry["run_id"], entity=self.registry["entity"], project=self.registry["project"], name=evaluation_run_name(self.registry), config=cfg, tags=_evaluation_run_tags(self.registry), job_type="evaluation-curve", resume=resume, dir=str(self.run_dir), reinit=True)
