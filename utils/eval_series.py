...
def backbone_display_label(identity, *, compact=False):
    ...
    known = {
        ...
        "rwgao_b-brown-university/ambi/mey3rxj8": ("TD-AMBI reward/Q-scale prior", "TD-AMBI reward/Q-scale"),
        "rwgao_b-brown-university/ambi/zdnoyhbt": ("TD-AMBI entropy/Q-scale prior", "TD-AMBI entropy/Q-scale"),
        "rwgao_b-brown-university/ambi/hw0nlj29": ("TD-AMBI entropy/auto-temperature prior", "TD-AMBI entropy/auto-temp"),
        "rwgao_b-brown-university/ambi/jirflxz1": ("TD-AMBI reward/auto-temperature prior", "TD-AMBI reward/auto-temp"),
    }
...
def _planner_display_label(planner, *, compact=False):
    ...
    if kind in ("sac", "xqc"):
        ...
        if settings.get("inner_update_timing") == "step" and rounds:
            denominator = rounds * settings.get("inner_rollout_horizon", 1)
            doses = [number(settings.get(f"inner_{component}_updates_per_action", 0) / denominator)
                     for component in ("critic", "actor", "temperature")]
            parts.append("/".join(prefix + dose for prefix, dose in zip("CAT", doses)) + "/step")
        else:
            parts.append("/".join(prefix + dose for prefix, dose in zip("CAT", doses)))
        if settings.get("sac_actor_loss_scale_mode") == "tdmpc2_percentile_range":
            parts.append("adaptive S" if settings.get("inner_actor_loss_scale_update") == "per_update" else "fixed S")
        if "inner_temperature_mode" in settings:
            parts.append("adaptive alpha" if settings.get("inner_temperature_mode") == "auto" else "fixed alpha")
        ...
        if not compact:
            ...
            if backend == "tdmpc2_mppi_over_frozen_xqc":
                ...
                target = planner.get("semantics", {}).get("source_critic_target", "entropy_augmented")
                parts.append("online reward Q" if target == "reward_only" else "online entropy-augmented Q")
    return " ".join(parts)
