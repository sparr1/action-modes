...
def _validate_checkpoint_overrides(alg_params, run_params, location):
    ...
    forbidden = [
        ...
        and key not in _CHECKPOINT_INNER_PARAMS
    ]
...
def _apply_alg_overrides(alg_params, overrides):
    # Null removes inherited schedule controls. Temperature clipping separately
    # accepts literal None to disable clipping, matching the outer optimizer.
    for key, value in copy.deepcopy(overrides).items():
        if value is None and key != "inner_temperature_grad_clip_norm":
...
def resolve_preset(matrix_path, selector, matrix=None, *, checkpoint_context=None):
    ...
    if matrix["base_alg_config"] == "checkpoint":
        ...
        source_algorithm = algorithm_config.get("alg")
        ...
        if source_algorithm == "TDMPC2/TDMPC2Baseline" and selected_operator == "tdambi":
            ...
            algorithm_config["alg"] = "TDAMBI/TDAMBI"
        elif source_algorithm != "AMBITDMPC2/AMBITDMPC2":
            raise PresetMatrixError(
                "Checkpoint-based presets require an AMBITDMPC2 checkpoint, "
                "or a native TDMPC2 checkpoint with the TDAMBI controller."
            )
        elif selected_operator == "tdambi":
            raise PresetMatrixError("TDAMBI requires a native TDMPC2 checkpoint, not an AMBI backbone.")
        environment = {
    ...
    return {
        ...
        **({"source_algorithm": source_algorithm} if matrix["base_alg_config"] == "checkpoint"
           and source_algorithm == "TDMPC2/TDMPC2Baseline" else {}),
    }
