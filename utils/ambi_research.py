...
def resolve_preset(matrix_path, selector, matrix=None, *, checkpoint_context=None):
    ...
    if matrix["base_alg_config"] == "checkpoint":
        ...
        if algorithm_config.get("alg") not in {"AMBITDMPC2/AMBITDMPC2", "AMBIXQC/AMBIXQC"}:
            raise PresetMatrixError("Checkpoint-based presets require an AMBITDMPC2 or AMBIXQC checkpoint.")
        if algorithm_config["alg"] == "AMBIXQC/AMBIXQC":
            overrides = set(matrix.get("shared_alg_params", {})) | set(variant.get("alg_params", {}))
            unsupported = sorted(key for key in overrides
                                 if key.startswith("inner_") and key not in _XQC_CHECKPOINT_INNER_PARAMS)
            if unsupported:
                raise PresetMatrixError(
                    f"AMBI-XQC checkpoint presets contain unsupported inner controls/probes: {unsupported}."
                )
        environment = {
