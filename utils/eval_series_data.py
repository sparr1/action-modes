...
def scientific_identity(algorithm, controller, commit, dirty=False, source_sha256=None):
    ...
    if algorithm == "TDAMBI/TDAMBI":
        paths = ["RL/TDAMBI.py", "RL/AMBITDMPC2.py", "RL/TDMPC2.py",
                 "RL/tdmpc2_core/ambi_agent.py", "RL/tdmpc2_core/agent.py",
                 "RL/tdmpc2_core/inner_improvement.py", "RL/tdmpc2_core/inner_trace.py",
                 "utils/ambi_research.py", "utils/checkpoint_context.py"]
    elif "XQC" in algorithm:
...
def _tdambi_planner(config, algorithm, action_rule):
    """Identity of native TD-MPC2 updates, independent of measured local scales."""
    keys = (
        ...
        "inner_replay_sampling", "inner_actor_lr", "inner_critic_lr",
        "inner_actor_adam_eps", "inner_adam_eps", "inner_actor_grad_clip_norm",
        "inner_critic_grad_clip_norm", "inner_critic_target_tau",
        "inner_critic_target_update_interval", "tdambi_entropy_coef",
        "tdambi_value_coef", "tdambi_scale_tau", "discount", "episodic",
        "dropout", "log_std_mapping", "log_std_min", "log_std_max",
        "inner_log_std_mapping", "inner_log_std_min", "inner_log_std_max",
    ...
    if config.get("inner_actor_initial_std") is not None:
        settings["inner_actor_initial_std"] = config["inner_actor_initial_std"]
    initialization = {}
    for component in ("actor", "critic"):
        key = f"inner_{component}_initialization"
        value = str(config.get(key, "prior")).lower()
        _require(value in {"prior", "random"}, f"Unknown TDAMBI {key}")
        initialization[component] = value
        if value != "prior":
            settings[key] = value
    adaptation = str(config.get("inner_critic_adaptation", "clone")).lower()
    _require(adaptation in {"clone", "lora_rl"}, "Unknown TDAMBI critic adaptation")
    if adaptation == "lora_rl":
        lora = publication_lora_identity(config)
        settings["inner_critic_adaptation"] = adaptation
        settings.update({key: value for key, value in lora.items()
                         if key.startswith("inner_critic_lora_")})
    ...
    semantics = {
        ...
        "target_initialization": "saved_outer_target",
        "temperature": "fixed_native_entropy_coefficient_no_temperature_optimizer",
        "scale_initialization": "first_replay_frozen_prior_online_mean_pair_q_p95_minus_p05_floor_1",
        "scale_calibration_sampling": "independent_stream_with_replacement_one_inner_batch",
    ...
    if initialization["critic"] == "random" or adaptation == "lora_rl":
        target_initialization = str(config.get("inner_critic_target_initialization", "online")).lower()
        _require(target_initialization == "online",
                 "TDAMBI random or LoRA critic requires online target initialization")
        semantics["target_initialization"] = (
            "merged_online_critic" if adaptation == "lora_rl" else "online_critic"
        )
    # Prior/dense TDAMBI always used the saved outer target. Historical bundles
    # sometimes recorded the shared SAC 'online' default, which was inert for
    # this operator; it must not become a different historical planner now.
    # Historical bundles omitted this option and always calibrated. Current
    # checkpoint resolution supplies the explicit default before preflight.
    # The requested rule belongs to a curve's identity; checkpoint-specific
    # values and whether the conditional fallback ran remain diagnostics.
    scale_initialization = config.get("tdambi_scale_initialization", "calibrate")
    _require(scale_initialization in {"checkpoint_or_calibrate", "checkpoint", "calibrate"},
             "Unknown TDAMBI scale initialization policy")
    if scale_initialization != "calibrate":
        settings["tdambi_scale_initialization"] = scale_initialization
        semantics["scale_initialization"] = (
            "saved_checkpoint_scale_else_first_collection_calibration"
            if scale_initialization == "checkpoint_or_calibrate"
            else "saved_checkpoint_scale_required"
        )
        semantics["scale_calibration_sampling"] = (
            "missing_checkpoint_scale_only_independent_stream_one_inner_batch"
            if scale_initialization == "checkpoint_or_calibrate" else "none"
        )
...
def planner_identity(config, result, algorithm, action_rule):
    ...
    if result.get("controller") in {"prior", "policy_prior"} or operator == "none":
        identity = {"type": "prior", "action_rule": action_rule}
        if config.get("inner_actor_source", "sac") != "sac":
            identity["settings"] = {"inner_actor_source": config["inner_actor_source"]}
        return identity
    if operator == "tdambi":
    ...
    if operator == "mppi":
        ...
        active.pop("inner_critic_source", None)
    # Historical inner networks always started from the learned priors. Keep
    # that omitted/default spelling out of existing curve identities.
    for component in ("actor", "critic"):
        key = f"inner_{component}_initialization"
        value = active.get(key, "prior")
        ...
        if value == "prior":
            ...
            active[key] = value
    # Older solves always updated after collection. Omitted and explicitly
    # resolved defaults identify the same execution; step timing is distinct.
    ...
    shared = {key: config[key] for key in ("discount", "episodic", "grad_clip_norm",
              ...
              if key in config}
    if operator == "sac" and config.get("inner_critic_source", "sac") == "aux_return":
        actor_config = config.get("aux_return_actor_cfg", {})
        for key in ("sac_actor_loss_scale_mode", "sac_actor_loss_scale_tau"):
            value = actor_config.get(key, config.get(f"aux_return_{key}", config.get(key)))
            if value is not None:
                shared[key] = value
    if operator == "mppi":
        shared.update({key: config[key] for key in ("mppi_terminal_q_reduction", "q_pair_size")
                       if key in config})
...
def _episode_protocol(protocol, seeds, config=None, action_rule=None):
    keys = {"environment", "env_wrapper", "env_wrappers", "observation", "action_rule",
            "controller_seed", "seed_scheme", "max_steps", "prior_reference"}
...
def _metrics(episodes):
    ...
    if all(ep.get("togo_probe_seconds") is not None for ep in episodes):
        ...
        metrics["work/togo_probe_model_steps"] = sum(
            _number(ep["togo_probe_model_steps"], "to-go probe model steps") for ep in episodes)
...
def validate_tdmpc2_prior_pairing(prior, protocol, seeds=None):
    """Validate common episode conditions while retaining each controller RNG."""
    native = prior["identity"]["protocol"]
    trial = prior["provenance"]["checkpoint_metadata"]["trial_run_params"]
    observation = trial.get("resolved_runtime", {}).get("observation", {}).get(
        "mode", trial.get("alg_params", {}).get("obs", "state"))
    compatible = (
        protocol.get("prior_reference") == "native_tdmpc2_prior_mean-v1"
        and native["environment"] == protocol.get("environment")
        and native["max_steps"] == protocol.get("max_steps")
        and native["controller_seed"] == protocol.get("controller_seed")
        and native["action_rule"] == protocol.get("action_rule") == "tanh_mean"
        and observation == protocol.get("observation") == "state"
        and trial.get("env_wrappers", []) == protocol.get("env_wrappers", [])
        and trial.get("env_wrapper") == protocol.get("env_wrapper")
        and native["seed_scheme"] == "independent fixed namespaced stream per environment seed"
    )
    _require(compatible, "Prior reference environment/action/seed protocol does not match")
    if seeds is not None:
        _require(sorted(seeds) == native["environment_seeds"],
                 "Prior reference environment seeds do not match requested episodes")
    for episode in prior["episodes"]:
        payload = f"tdmpc2-mppi-eval:{int(native['controller_seed'])}:policy_prior_mean:{int(episode['seed'])}"
        expected_seed = int.from_bytes(hashlib.sha256(payload.encode()).digest()[:8], "big") % 2**32
        _require(episode.get("solver_seed") == expected_seed,
                 "Prior reference controller RNG seed does not match the native protocol")


def _bundle_prior_reference(manifest, path, checkpoint, source):
    """Revalidate a portable native-prior reference before publication."""
    reference = manifest.get("reference", {})
    if reference.get("format") != "tdmpc2-paired-prior":
        return {}, {}
    _require(reference.get("checkpoint_sha256") == checkpoint["sha256"] and
             reference.get("checkpoint_step") == checkpoint["step"], "Reference checkpoint mismatch")
    _require(reference.get("source_run") == source, "Reference backbone mismatch")
    artifacts = {}
    for name, digest in reference.get("files", {}).items():
        file = (path.parent / name).resolve()
        _require(file.is_relative_to(path.parent) and name.startswith("reference/"),
                 "Reference artifact path escapes bundle")
        _require(file.is_file() and _file_hash(file) == digest, "Reference artifact hash mismatch")
        artifacts[name] = str(file)
    _require("reference/paired.json" in artifacts, "Missing exact prior reference file")
    _require(_file_hash(artifacts["reference/paired.json"]) == reference.get("source_result_sha256"),
             "Reference result hash mismatch")
    inventory = next((file for name, file in artifacts.items()
                      if name.startswith("reference/provenance/checkpoint-inventory-")), None)
    records = normalize_tdmpc2(artifacts["reference/paired.json"], source_run=source,
                               checkpoint_inventory=inventory)
    prior, = [record for record in records if record["controller"] == "policy_prior"]
    _require(prior["identity"]["protocol"] == reference.get("source_protocol") and
             prior["identity"]["science"] == reference.get("source_science") and
             prior["episodes"] == reference.get("episodes"), "Reference provenance differs from saved source")
    actual_protocol = {key: value for key, value in manifest.get("protocol", {}).items()
                       if key != "root_bank_id"}
    _require(actual_protocol == reference.get("evaluation_protocol"), "Reference pairing protocol mismatch")
    validate_tdmpc2_prior_pairing(prior, actual_protocol)
    return {episode["seed"]: episode["return"] for episode in prior["episodes"]}, artifacts
...
def normalize_bundle(path, *, checkpoint_inventory=None, source_run=None):
    ...
    reference_returns, reference_files = _bundle_prior_reference(manifest, path, checkpoint, source)
    ...
    for run in manifest["runs"]:
        ...
        if reference_returns:
            _require(set(reference_returns) == set(seeds), "Reference episode seeds differ from evaluation")
            for episode in episodes:
                expected = episode["return"] - reference_returns[episode["seed"]]
                _require(episode.get("paired_return_delta") == expected,
                         "Paired return delta differs from exact prior reference")
        elif config.get("inner_operator") == "tdambi":
            _require(not any("paired_return_delta" in episode for episode in episodes),
                     "TDAMBI paired gains require a validated native prior reference")
        ...
        identity = {"backbone": source, "planner": planner, "protocol": protocol,
                    "science": scientific_identity(algorithm, controller, code.get("commit"),
                                                     code.get("dirty"), code.get("source_sha256"))}
        artifacts = {"manifest.json": str(path), **extra_files, **reference_files}
        missing = []
