...
def planner_identity(config, result, algorithm, action_rule):
    ...
    # These fields made pre-existing behavior explicit in newer resolvers.
    # Keep omitted historical defaults compatible without erasing alternatives
    # that change losses, target initialization, or update ordering.
    historical_defaults = {
    ...
    for key, default in historical_defaults.items():
        if active.get(key, default) == default:
            active.pop(key, None)
    if active.get("inner_actor_adam_eps") == config.get("inner_adam_eps"):
        active.pop("inner_actor_adam_eps", None)
    # S cadence has no effect when Q normalization is disabled. With scaling
    # enabled, retain even the frozen policy explicitly, as well as adaptive S.
    if config.get("sac_actor_loss_scale_mode", "none") == "none":
        active.pop("inner_actor_loss_scale_update", None)
    else:
        active.setdefault("inner_actor_loss_scale_update", "per_action")
    if operator == "xqc":
...
def identity_for_ambi_checkpoint(checkpoint, resolved, protocol, seeds, code, *, path,
    ...
    if evaluation_controller is None and resolved.get("evaluation_controller"):
        ...
        description.critic_target = str(agent.cfg.outer_critic_target)
...
def descriptive_label(identity, selector=None):
    ...
    if planner["type"] == "mppi":
        ...
        if planner.get("backend") == "tdmpc2_mppi_over_frozen_xqc":
            ...
            title += " | online reward Q" if target == "reward_only" else " | online entropy-augmented Q"
        return prefix + title
