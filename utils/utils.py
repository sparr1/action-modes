...
def listify(obj, wrap=True):
    if (
        ...
        return [
            {k: listify(v, False) for k, v in obj.items()},
        ]
...
def _convert_arrays_recursively(obj):
    """Recursively convert numpy arrays to lists while preserving structure."""
    if isinstance(obj, np.ndarray):
        return obj.tolist()
    elif isinstance(obj, np.generic):
        return obj.tolist()
    elif isinstance(obj, dict):
        return {k: _convert_arrays_recursively(v) for k, v in obj.items()}
    elif isinstance(obj, (list, tuple)):
        return [_convert_arrays_recursively(x) for x in obj]
    else:
        return obj
