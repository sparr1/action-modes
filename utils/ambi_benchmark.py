...
def code_identity():
    ...
    def git(*args):
        # Use posix_spawn: macOS rejects forked children after Torch/OpenMP
        # initializes. Absolute executable plus -C avoids subprocess cwd/fork.
        executable = shutil.which("git")
        if executable is None:
            raise OSError("Git executable not found")
        return subprocess.check_output([executable, "-C", str(root), *args],
            stderr=subprocess.DEVNULL, close_fds=False).decode().strip()
