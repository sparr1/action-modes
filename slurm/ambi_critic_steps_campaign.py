"""Vary critic fitting in the unchanged J1/H1 closed-loop refinement learner.

The immutable manifest owns eighteen new cells and three verified historical
C32 cells. Workers and mergers stay offline; the complete-panel reporter alone
publishes a new comparison. C0 still performs all four actor updates.
"""
...
CELL_IDS = [f"c{count}-s{seed}" for count in CRITIC_UPDATES for seed in CONTROLLERS]
...
def load_campaign(path):
    ...
    _require(data.get("critic_updates") == CRITIC_UPDATES and data.get("rounds") == 1,
             "Campaign must preserve the declared critic-step sweep at J1")
    ...
    _require([cell.get("cell_id") for cell in cells] == CELL_IDS, "Campaign must contain the ordered 21 cells")
    destinations = set()
    for cell in cells:
        count, j, seed = cell.get("critic_updates"), cell.get("rounds"), cell.get("controller_seed")
        _require(type(count) is int and count in CRITIC_UPDATES and type(j) is int and j == 1
                 and type(seed) is int and seed in CONTROLLERS
                 and cell["cell_id"] == f"c{count}-s{seed}", "Cell coordinates differ from its identity")
        ...
        _require(cell.get("reused") is (count == 32), "Only the three completed C32 cells are reused")
        ...
        expected["inner_critic_updates_per_round"] = count
        _require(config["shared_alg_params"] == expected, "Only critic update count may change the learner")
        ...
        if cell["reused"]:
            _require(all(isinstance(cell.get(key), str) and re.fullmatch(r"[0-9a-f]{64}", cell[key])
                     ...
                     "Historical C32 cells require pinned manifest and seal file SHA256s")
        cell["matrix"] = str(matrix_path)
    return data


def task_cells(data):
    """Longest critic fits first, with two seed shards for C64."""
    return [(index, shard) for index, cell in sorted(enumerate(data["cells"]),
            key=lambda item: (-item[1]["critic_updates"], item[1]["controller_seed"]))
...
def _receipt(data, path, cell, shard, row, smoke):
    return dict(status="complete", cell_id=cell["cell_id"], critic_updates=cell["critic_updates"], rounds=cell["rounds"],
...
def run_worker(campaign, task_index, smoke=False):
    ...
    if smoke:
        _run(["-m", "pytest", "-q", "tests/test_ambi_critic_steps_campaign.py",
...
def merge_cell(campaign, cell_index, smoke=False, shard_indices=None):
    ...
    if cell["reused"]:
        ...
        # before any production jobs depend on a successful reuse receipt.
        from slurm.ambi_critic_steps_report import load_cell
        ...
        receipt = dict(status="complete", cell_id=cell["cell_id"], critic_updates=cell["critic_updates"], reused=True, bundle=str(bundle),
        ...
        receipt = dict(status="complete", cell_id=cell["cell_id"], critic_updates=cell["critic_updates"], reused=False, smoke=smoke,
