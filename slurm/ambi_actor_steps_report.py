"""Strict complete-panel actor-update sweep, with streamed training traces.
...
validate. A4 is an exact historical reference; every setting fits the critic32 times.
"""
...
SCHEMA = "ambi-actor-steps-comparison-v1"
...
AXES = ["actor_updates_per_decision", "optimizer_updates_per_decision", "control_seconds_per_decision"]
TRAINING_METRICS = ("critic_loss", "td_error_abs_mean", "critic_grad_norm", "q_mean",
                    "q_abs_mean", "q_target_mean", "q_target_clip_fraction")


ACTOR_TRAINING_METRICS = ("actor_loss", "actor_grad_norm", "actor_q_mean", "actor_q_mean_all", "actor_q_min_all",
                          "actor_q_mean_all_minus_min_all", "actor_entropy", "actor_scaled_entropy", "actor_entropy_bonus",
                          "actor_pre_tanh_abs_mean", "actor_pre_tanh_abs_max", "actor_pre_tanh_abs_ge_7p6_fraction",
...
def validate_campaign(campaign):
    ...
    require(campaign.get("actor_updates") == ACTOR_UPDATES and campaign.get("critic_updates") == 32
            and campaign.get("rounds") == 1, "Wrong actor-count sweep")
    ...
    require({(c["actor_updates"], c["controller_seed"]) for c in cells}
            == {(n, s) for n in ACTOR_UPDATES for s in CONTROLLERS}, "Incomplete actor/controller grid")
    for c in cells:
        require(c["seeds"] == SEEDS and c["rounds"] == 1 and c["critic_updates"] == 32
                and Path(c["bundle"]).is_absolute(), "Wrong seeds, fixed critic count, rounds or relative bundle")
        require(bool(c.get("reused")) == (c["actor_updates"] == 4), "Only all three historical A4 cells are reused")
        if c["reused"]:
...
def training_trace_summaries(cell, run, seal, episode_rows):
    """Validate complete optimizer coordinates and stream per-episode step moments.

    Both critic and actor measurements precede the indexed optimizer update.
    Original gzip traces remain in sealed source bundles with portable hashes.
    """
    root, count = Path(cell["bundle"]).resolve(), cell["actor_updates"]
    ...
    for relative in paths:
        ...
        with gzip.open(path, "rt", encoding="utf-8") as handle:
            for line in handle:
                ...
                family = "critic" if critic else "actor"
                step = row[family + "_updates"]
                if critic:
                    require(type(step) is int and 1 <= step <= 32 and row["actor_updates"] == 0,
                            "Wrong critic update counter")
                    require(masks[0] == (1 << (step - 1)) - 1 and masks[1] == 0,
                            "Duplicate or out-of-order critic update")
                    masks[0] |= 1 << (step - 1)
                else:
                    require(type(step) is int and 1 <= step <= count and row["critic_updates"] == 32,
                            "Wrong actor update counter")
                    require(masks[0] == (1 << 32) - 1 and masks[1] == (1 << (step - 1)) - 1,
                            "Duplicate or out-of-order actor update")
                    masks[1] |= 1 << (step - 1)
                values = accumulated.setdefault((key[0], family, step), {})
                for name in TRAINING_METRICS if critic else ACTOR_TRAINING_METRICS:
                    value = finite(row["metrics"].get(name), "Missing/nonfinite " + family + " training metric")
    ...
    require(set(coordinates) == expected and all(masks == [(1 << 32) - 1, (1 << count) - 1] for masks in coordinates.values()),
            "Incomplete critic/actor trace coverage")
    summaries = {}
    for episode in episode_rows:
        families = {}
        for family, steps in (("critic", 32), ("actor", count)):
            points = []
            for step in range(1, steps + 1):
                raw = accumulated[f"seed-{episode['seed']}", family, step]
                require(all(s["count"] == episode["length"] for s in raw.values()), "Incomplete training metric coverage")
                points.append({family + "_updates": step, family + "_update_index": step,
                               "measurement": "pre_update_minibatch", "metrics": {name: {
        ...
        summaries[episode["seed"]] = families
...
def load_cell(cell):
    ...
    run, count = runs[0], cell["actor_updates"]
    ...
    expected = {"inner_rounds": 1, "inner_rollout_horizon": 1, "inner_rollouts_per_round": 128,
                "inner_actor_updates_per_round": count, "inner_critic_updates_per_round": 32,
    ...
    require(all(params.get(k) == v and config["alg_params"].get(k) == v for k, v in expected.items()),
            "Wrong actor-count learner schedule")
    ...
    for row in episode_rows:
        ...
        record = {"cell_id": cell["cell_id"], "actor_updates": count, "critic_updates": 32, "rounds": 1,
                  ...
                  "critic_training_steps": training[row["seed"]]["critic"],
                  "actor_training_steps": training[row["seed"]]["actor"]}
        ...
        for name, key in WORK.items():
            ...
            require(value == {"actor_updates": count, "critic_updates": 32, "model_transitions": 128}[name],
                    f"Unexpected realized {name}")
            record[name + "_per_decision"], record[name + "_per_episode"] = value, value * row["length"]
        record["optimizer_updates_per_decision"] = count + 32
        record["optimizer_updates_per_episode"] = (count + 32) * row["length"]
        ...
        for point in points:
            r = point["round_index"]
            require(point["actor_updates"] == count * r and point["critic_updates"] == 32 * r,
    ...
    science = {k: v for k, v in params.items() if k != "inner_actor_updates_per_round"}
    compatibility = {"checkpoint": checkpoint, "protocol": {k: v for k, v in protocol.items() if k != "controller_seed"},
                     "alg_params_except_actor_updates": science, "runtime": code["runtime"], "probe": probe,
    ...
    return {"cell": cell, "rows": output, "prior": prior, "compatibility": compatibility,
            "identity": {"cell_id": cell["cell_id"], "actor_updates": count, "critic_updates": 32, "rounds": 1,
...
def build_report(campaign, loaded, resamples=2000):
    ...
    index = {(r["actor_updates"], r["controller_seed"], r["seed"]): r for r in rows}
    require(len(rows) == 240 and len(index) == 240, "Incomplete or duplicate episode grid")
    numeric = ["return", "gain_vs_prior", "gain_vs_a1", "gain_vs_a4", "length", "control_seconds",
    ...
    for row in rows:
        count, c, seed = row["actor_updates"], row["controller_seed"], row["seed"]
        row["prior_return"] = priors[0][seed][0]
        row["gain_vs_prior"] = row["return"] - row["prior_return"]
        for target in ACTOR_UPDATES:
            require(row["solver_seed"] == index[target, c, seed]["solver_seed"], "Unpaired controller RNG seeds")
        row["gain_vs_a1"] = row["return"] - index[1, c, seed]["return"]
        row["gain_vs_a4"] = row["return"] - index[4, c, seed]["return"]
    by_episode, summaries, probes, training, actor_training, contrasts, availability = [], [], [], [], [], [], {}
    for count in ACTOR_UPDATES:
        group = [r for r in rows if r["actor_updates"] == count]
        keys = sorted(group[0]["model_metrics"])
        require(all(set(r["model_metrics"]) == set(keys) for r in group), "Diagnostic availability differs within A")
        availability[str(count)] = keys
        averaged = []
        for seed in seeds:
            selected = [index[count, c, seed] for c in controllers]
            averaged.append({"actor_updates": count, "seed": seed,
        ...
        summaries.append({"actor_updates": count, "reused": count == 4,
        ...
        for boundary in (0, 1):
            ...
            probes.append({"configured_actor_updates": count, "round_index": boundary, "actor_updates": boundary * count,
                           "critic_updates": boundary * 32,
                           "metrics": _summaries(episode_metrics, sorted(episode_metrics[0]), resamples)})
        for family, steps, metric_keys, destination in (("critic",32,TRAINING_METRICS,training),
                                                          ("actor",count,ACTOR_TRAINING_METRICS,actor_training)):
            for step in range(1, steps + 1):
                episode_metrics = [{k: statistics.fmean(index[count, c, seed][family + "_training_steps"][step - 1]["metrics"][k]["mean"]
                                    for c in controllers) for k in metric_keys} for seed in seeds]
                destination.append({"configured_actor_updates": count, family + "_updates": step,
                                    family + "_update_index": step, "measurement": "pre_update_minibatch",
                                    "metrics": _summaries(episode_metrics, metric_keys, resamples)})
        references = {4: ["versus_historical_a4"]}
        previous = ACTOR_UPDATES[ACTOR_UPDATES.index(count)-1] if count != ACTOR_UPDATES[0] else None
        if previous is not None:
            references.setdefault(previous, []).append("adjacent_actor_dose")
        for reference, roles in references.items():
            if reference == count:
                continue
            differences = [statistics.fmean(index[count, c, s]["return"] - index[reference, c, s]["return"] for c in controllers) for s in seeds]
            contrasts.append({"target_actor_updates": count, "reference_actor_updates": reference, "roles": roles,
    ...
    return {"schema": SCHEMA, "status": "complete", "attempt_label": campaign["attempt_label"],
            "campaign_sha256": digest(campaign), "actor_updates": ACTOR_UPDATES, "critic_updates": 32, "rounds": 1,
            ...
            "diagnostic_scope": "Controller-specific states. Different actor-count settings may visit different states and collect different subsequent replay. Training curves use fresh pre-update sampled minibatches and sampled bootstrap targets, not a fixed held-out evaluation set. Critic loss is two-hot distributional cross-entropy; decoded TD error is in reward/Q units.",
            "timing_semantics": "Recorded control time already excludes model probe time and warmup; no further subtraction. A4 timing is historical and is not paired under identical machine load.",
            "constant_work": {"critic_updates_per_decision": 32, "optimization_model_transitions_per_decision": 128,
            ...
            "model_probes": probes, "critic_training": training, "actor_training": actor_training, "episode_averages": by_episode, "paired_rows": rows}
...
def render_html(report):
    ...
    return """<!doctype html><meta charset="utf-8"><title>Actor updates and episode control</title>
...
<h1>Actor updates during repeated mean-action control</h1><p id="status"></p>
...
<section><b>Critic training trajectory</b><p><select id="trainingmetric"></select> against critic update index (measurement before update)</p><svg id="training" viewBox="0 0 1050 360"></svg><small>Each loss/TD-error measurement uses the minibatch before that update, not a post-update held-out error. Each curve uses the states visited by its own controller. The critic receives32 updates in every setting.</small></section>
<section><b>Actor training trajectory</b><p><select id="actormetric"></select> against actor update index (measurement before update)</p><svg id="actortraining" viewBox="0 0 1050 360"></svg><small>Sampled training actions and gradients before each update; saturation is not the fraction of executed mean actions at a bound.</small></section>
<section><b>Model return before and after adaptation</b><p><select id="probemetric"></select> against actual actor updates</p><svg id="probes" viewBox="0 0 1050 360"></svg><small>Initial: zero actor and critic updates. Final: A actor updates and32 critic updates. Sampled-policy probes on controller-specific states.</small></section>
...
<section><button id="download">Download complete report JSON (240 paired episodes, critic/actor-step summaries and source trace checksums)</button><p id="timing"></p><p id="storage"></p></section>
...
document.getElementById('status').textContent=`${D.attempt_label}: ${D.status}. ${D.complete_cells??12}/12 cells. Three controller repetitions × twenty environment episodes per A; A4 reused exactly.`;
...
options('metric',[...Object.keys(S[0].metrics).map(k=>['metrics/'+k,k]),...Array.from(new Set(S.flatMap(s=>Object.keys(s.model_metrics)))).sort().map(k=>['model_metrics/'+k,'Episode diagnostic: '+k])]);options('axis',[['actor_updates_per_decision','actor updates / decision'],['optimizer_updates_per_decision','total optimizer updates / decision'],['control_seconds_per_decision','recorded control seconds / decision']]);options('trainingmetric',Object.keys(D.critic_training[0].metrics).map(k=>[k,k]));options('probemetric',Object.keys(D.model_probes[0].metrics).map(k=>[k,k]));options('actormetric',Object.keys(D.actor_training[0].metrics).map(k=>[k,k]));document.getElementById('actormetric').value='actor_grad_norm';document.getElementById('trainingmetric').value='td_error_abs_mean';document.getElementById('probemetric').value='togo_return_gain_vs_outer';
function main(){let [family,key]=document.getElementById('metric').value.split('/'),axis=document.getElementById('axis').value;draw('chart',[{label:'A1 / A2 / A4 (historical) / A8',points:S.filter(s=>s[family][key]).map(s=>({x:s.metrics[axis].mean,m:s[family][key],label:'A'+s.actor_updates}))}],key.includes('gain'),key==='return'?D.prior.summary.mean:null)};
function train(){let key=document.getElementById('trainingmetric').value;draw('training',D.actor_updates.map(c=>({label:'A'+c+(c===4?' (historical)':''),points:D.critic_training.filter(s=>s.configured_actor_updates===c).map(s=>({x:s.critic_updates,m:s.metrics[key]}))})))};
function actors(){let key=document.getElementById('actormetric').value;draw('actortraining',D.actor_updates.map(a=>({label:'A'+a+(a===4?' (historical)':''),points:D.actor_training.filter(s=>s.configured_actor_updates===a).map(s=>({x:s.actor_updates,m:s.metrics[key]}))})))};
function probes(){let key=document.getElementById('probemetric').value;draw('probes',D.actor_updates.map(a=>({label:'A'+a+(a===4?' (historical)':''),points:D.model_probes.filter(s=>s.configured_actor_updates===a).map(s=>({x:s.actor_updates,m:s.metrics[key],label:'critic updates '+s.critic_updates}))})),key.includes('gain'))};
document.getElementById('metric').onchange=main;document.getElementById('axis').onchange=main;document.getElementById('trainingmetric').onchange=train;document.getElementById('probemetric').onchange=probes;document.getElementById('actormetric').onchange=actors;main();train();actors();probes();
document.getElementById('summary').innerHTML=table(['A','Return [95% CI]','Gain vs prior','Gain vs A1','Gain vs A4','Episode-mean SD','60-return SD'],S.map(s=>[s.actor_updates+(s.reused?' (reused)':''),range(s.metrics.return),range(s.metrics.gain_vs_prior),range(s.metrics.gain_vs_a1),range(s.metrics.gain_vs_a4),fmt(s.metrics.return.std),fmt(s.raw_episode_return_std)]));document.getElementById('contrasts').innerHTML=table(['Contrast','Difference [95% CI]'],D.contrasts.map(c=>[`A${c.target_actor_updates} − A${c.reference_actor_updates}`,range(c.summary)]));
document.getElementById('scope').textContent=D.return_units+'. '+D.diagnostic_scope;document.getElementById('constant').textContent='Fixed: 128 model transitions,32 critic updates, H1, J1, B256 per decision.';document.getElementById('timing').textContent=D.timing_semantics;document.getElementById('storage').textContent=D.training_trace_storage;
...
def inspect(campaign, loaded, campaign_file_sha256):
    ...
    for cell in campaign["cells"]:
        ...
        try:
            ...
            expected = {"status": "complete", "cell_id": cell["cell_id"], "actor_updates": cell["actor_updates"], "critic_updates": 32, "rounds": 1,
...
def publish_science(run, wandb, report, output):
    ...
    run.define_metric("episode_diagnostics/*", step_metric="compute/actor_updates_per_decision")
    for count in ACTOR_UPDATES:
        run.define_metric(f"probes/a{count}/actor_updates")
        run.define_metric(f"model_probes/a{count}/*", step_metric=f"probes/a{count}/actor_updates")
        for family in ("critic", "actor"):
            run.define_metric(f"training/a{count}/{family}_update_index")
            run.define_metric(f"{family}_training/a{count}/*", step_metric=f"training/a{count}/{family}_update_index")
    for row in report["summaries"]:
        values = {"compute/" + axis: row["metrics"][axis]["mean"] for axis in AXES}
        for axis in AXES:
            for name in ("return", "gain_vs_prior", "gain_vs_a1", "gain_vs_a4"):
    ...
    for row in report["model_probes"]:
        count = row["configured_actor_updates"]
        run.log({f"probes/a{count}/actor_updates": row["actor_updates"], f"probes/a{count}/critic_updates": row["critic_updates"],
                 **{f"model_probes/a{count}/{name}/{stat}": summary[stat]
                    for name, summary in row["metrics"].items() for stat in ("mean", "ci95_low", "ci95_high")}})
    for family in ("critic", "actor"):
        for row in report[family + "_training"]:
            count = row["configured_actor_updates"]
            run.log({f"training/a{count}/{family}_update_index": row[family + "_updates"],
                     **{f"{family}_training/a{count}/{name}/{stat}": summary[stat]
                        for name, summary in row["metrics"].items() for stat in ("mean", "ci95_low", "ci95_high")}})
    for contrast in report["contrasts"]:
        stem = f"contrasts/a{contrast['target_actor_updates']}_minus_a{contrast['reference_actor_updates']}"
        for stat in ("mean", "std", "ci95_low", "ci95_high"):
            run.summary[stem + "/" + stat] = contrast["summary"][stat]
    run.log({"comparison/report": wandb.Html(str(output / "report.html"), inject=False)})
    artifact = wandb.Artifact("actor-steps-" + digest(report["attempt_label"])[:16], type="actor-steps-comparison",
    ...
    run.summary.update({"comparison/report_sha256": fingerprint, "comparison/prior_return": report["prior"]["summary"]["mean"],
                        "comparison/historical_rows": 60, "comparison/training_measurement": "pre_update_minibatch"})
    ...
    try:
        if args.mode == "online":
            ...
            run = wandb.init(entity=campaign.get("wandb_entity", "rwgao_b-brown-university"),
                             ...
                             name="Actor updates with fixed critic/data counts | " + campaign["attempt_label"],
                             job_type="actor-steps-comparison", tags=["actor-steps", "full-episodes", "complete-panel"],
                             config={"comparison_schema": SCHEMA, "attempt_label": campaign["attempt_label"],
                                     "campaign_sha256": digest(campaign), "owner": args.owner, "actor_updates": ACTOR_UPDATES, "critic_updates": 32,
                                     ...
                                     "protocol": "Full500 mean decisions, J1 H1 N128 B256 C32; A=1/2/4/8; prior weights, alpha0 and saved Q scale",
