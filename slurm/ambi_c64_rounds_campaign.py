"""Extend C64 refinement to J2 with the unchanged closed-loop learner.

Three new C64/J2 cells join nine verified historical comparator cells. Workers and
mergers stay offline; only the complete-panel reporter publishes a comparison.
Only critic count and rounds vary; every round retains N128/H1/A4/B256.
"""
...
ROUNDS = [1, 2]
CELL_IDS = [f"c{count}-j{j}-s{seed}" for count in CRITIC_UPDATES for j in ROUNDS for seed in CONTROLLERS]
...
def load_campaign(path):
    ...
    _require(data.get("critic_updates") == CRITIC_UPDATES and data.get("rounds") == ROUNDS,
             "Campaign must preserve the declared C32/C64 and J1/J2 grid")
    ...
    for cell in cells:
        ...
        _require(type(count) is int and count in CRITIC_UPDATES and type(j) is int and j in ROUNDS
                 and type(seed) is int and seed in CONTROLLERS
                 and cell["cell_id"] == f"c{count}-j{j}-s{seed}", "Cell coordinates differ from its identity")
        ...
        _require(cell.get("reused") is (count == 32 or j == 1),
                 "All C32 and C64/J1 cells must be reused; only C64/J2 is new")
        ...
        _require(config["shared_alg_params"] == expected, "Only critic update count and rounds may change the learner")
        ...
        if cell["reused"]:
            _require(all(isinstance(cell.get(key), str) and re.fullmatch(r"[0-9a-f]{64}", cell[key])
                     ...
                     "Historical cells require pinned manifest and seal file SHA256s")
        cell["matrix"] = str(matrix_path)
    return data


def task_cells(data):
    """One twelve-task wave; five episodes per C64/J2 seed shard."""
...
def run_worker(campaign, task_index, smoke=False):
    ...
    if smoke:
        _run(["-m", "pytest", "-q", "tests/test_ambi_c64_rounds_campaign.py",
...
def merge_cell(campaign, cell_index, smoke=False, shard_indices=None):
    ...
    if cell["reused"]:
        ...
        from slurm.ambi_c64_rounds_report import load_cell
        ...
        receipt = dict(status="complete", cell_id=cell["cell_id"], critic_updates=cell["critic_updates"], rounds=cell["rounds"], reused=True, bundle=str(bundle),
        ...
        receipt = dict(status="complete", cell_id=cell["cell_id"], critic_updates=cell["critic_updates"], rounds=cell["rounds"], reused=False, smoke=smoke,
