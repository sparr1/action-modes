"""Run full-episode entropy arms with the unchanged ordinary J1/J2/J4 learner.

The immutable manifest owns 18 new cells and nine verified historical off cells.
Workers and mergers stay offline. The separate complete-panel reporter is the
only publisher; historical native curves remain untouched.
"""
...
ARMS = {"off": {"mode": "tdmpc2_scaled", "alpha": 0.0},
        "native": {"mode": "tdmpc2_scaled", "alpha": 0.0001},
        "squashed": {"mode": "squashed", "alpha": 0.0021}}
CELL_IDS = [f"{arm}-j{j}-c{seed}" for arm in ARMS for j in (1, 2, 4) for seed in CONTROLLERS]
...
def load_campaign(path):
    ...
    _require(data.get("arms") == ARMS, "Campaign must preserve the declared three entropy arms")
    ...
    _require([cell.get("cell_id") for cell in cells] == CELL_IDS, "Campaign must contain the ordered 27 cells")
    destinations = set()
    for cell in cells:
        arm, j, seed = cell.get("arm"), cell.get("rounds"), cell.get("controller_seed")
        _require(arm in ARMS and type(j) is int and j in (1, 2, 4)
                 and type(seed) is int and seed in CONTROLLERS
                 and cell["cell_id"] == f"{arm}-j{j}-c{seed}", "Cell coordinates differ from its identity")
        ...
        _require(cell.get("reused") is (arm == "off"), "Only the nine completed off cells are reused")
        ...
        reference = load_preset_matrix(ROOT / f"configs/research/round_scaling_j{j}_h1_200k.json")
        ...
        expected.update(inner_temperature=ARMS[arm]["alpha"], inner_actor_entropy_mode=ARMS[arm]["mode"])
        _require(config["shared_alg_params"] == expected, "Only entropy mode/coefficient may change the learner")
        ...
        if cell["reused"]:
            _require(all(isinstance(cell.get(key), str) and re.fullmatch(r"[0-9a-f]{64}", cell[key])
                     ...
                     "Historical off cells require pinned manifest and seal file SHA256s")
...
def task_cells(data):
    ...
    return [(index, shard) for index, cell in sorted(enumerate(data["cells"]),
            key=lambda item: (-item[1]["rounds"], list(ARMS).index(item[1]["arm"]), item[1]["controller_seed"]))
...
def _receipt(data, path, cell, shard, row, smoke):
    return dict(status="complete", cell_id=cell["cell_id"], arm=cell["arm"], rounds=cell["rounds"],
...
def run_worker(campaign, task_index, smoke=False):
    ...
    if smoke:
        _run(["-m", "pytest", "-q", "tests/test_ambi_entropy_episode_campaign.py",
              "tests/test_ambi_inner_entropy_modes.py", "tests/test_ambi_finite_horizon.py",
...
def merge_cell(campaign, cell_index, smoke=False, shard_indices=None):
    ...
    if cell["reused"]:
        ...
        receipt = dict(status="complete", cell_id=cell["cell_id"], arm=cell["arm"], reused=True, bundle=str(bundle),
        ...
        receipt = dict(status="complete", cell_id=cell["cell_id"], arm=cell["arm"], reused=False, smoke=smoke,
