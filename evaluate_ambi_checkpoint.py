    ...
    try:
        ...
        for seed in ([] if bank_only else seeds):
            ...
            while not (terminated or truncated):
                ...
                # MPPI exposes per-decision planner metrics, not SAC update traces.
                predict_options = ({"trace": trace} if trace is not None
                                   and model.cfg.inner_operator != "mppi" else {})
                started = time.perf_counter()
...
def evaluate_matrix(
    ...
    *,
    ...
    tdambi = any(item["algorithm_config"].get("alg") == "TDAMBI/TDAMBI"
                 for item in resolved_presets)
    mppi = any(item["algorithm_config"]["alg_params"].get("inner_operator") == "mppi"
               for item in resolved_presets)
    if mppi and (save_root_bank or root_bank_path or bank_only):
        raise ValueError("MPPI benchmark bundles support full episodes only.")
    ...
    if bundle_dir is not None:
        ...
        for resolved in resolved_presets:
            ...
            if params.get("inner_operator", "sac") not in {"none", "sac", "tdambi", "mppi"}:
